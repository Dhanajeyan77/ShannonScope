...
pub fn get_drive_info(target: &str) -> Result<DriveInfo, String> {
    ...
    // 16-byte CDB for ATA PASS-THROUGH (16)
    // Opcode 0x85, protocol 4 (PIO Data-in), T_DIR=1, byteblock=1, length=1, command=0xEC
    let mut cdb: [u8; 16] = [
    ...
    // We proceed even if SG_IO fails due to lack of root privileges (e.g. in tests)
    // We will parse the buffer if modified, or fallback to dummy data.
    
    // Serial is at word offset 10 (bytes 20-39)
    // Firmware is at word offset 23 (bytes 46-53)
