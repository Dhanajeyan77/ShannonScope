...
pub fn get_drive_info(target: &str) -> Result<DriveInfo, String> {
    ...
    #[cfg(target_os = "linux")]
    {
        let mut parent_target = target.trim_end_matches(|c: char| c.is_ascii_digit()).to_string();
        if parent_target.ends_with("p") {
            parent_target = parent_target.trim_end_matches('p').to_string();
        }

        let is_rotational = check_rotational(target);

        let file_res = File::open(&parent_target);
        if let Err(_) = file_res {
            return Ok(DriveInfo {
                target: target.to_string(),
                serial_number: "ACCESS_DENIED_OR_VIRTUAL".to_string(),
                firmware_revision: "UNAVAILABLE".to_string(),
                is_rotational,
            });
        }
        ...
        let ret = unsafe { libc::ioctl(fd, SG_IO, &mut io_hdr) };

        if ret < 0 {
            return Ok(DriveInfo {
                target: target.to_string(),
                serial_number: "SCSI_UNSUPPORTED".to_string(),
                firmware_revision: "UNAVAILABLE".to_string(),
                is_rotational,
            });
        }
        ...
        let final_serial = if serial.trim().is_empty() { "S_UNKNOWN".to_string() } else { serial };
...
fn parse_ata_string(bytes: &[u8]) -> String {
    ...
    for chunk in bytes.chunks(2) {
        if chunk.len() == 2 {
            if chunk[1] != 0 && chunk[1] >= 32 && chunk[1] <= 126 { s.push(chunk[1] as char); }
            if chunk[0] != 0 && chunk[0] >= 32 && chunk[0] <= 126 { s.push(chunk[0] as char); }
        }
    }
    s.trim().to_string()
}
...
pub fn check_rotational(target: &str) -> bool {
    ...
    let dev_name = Path::new(&parent_target).file_name().and_then(|n| n.to_str()).unwrap_or("");
